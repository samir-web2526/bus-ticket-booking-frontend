      ...
      <div className="relative h-full overflow-hidden rounded-[40px] bg-card border border-border hover:border-amber-500/20 shadow-sm hover:shadow-2xl transition-all duration-500 flex flex-col">
        ...
        <div className="flex items-center justify-between px-8 py-6 border-b border-border bg-muted/20">
          <div className="flex-1">
            <h3 className="font-bold text-foreground text-base">{bus.name}</h3>
          ...
          <div className="flex items-center gap-3">
            ...
            {isFull ? (
              <Badge className="bg-destructive/10 text-destructive border-none px-4 py-1.5 rounded-full text-xs font-medium">Full</Badge>
            ) : isLowSeats ? (
              <Badge className="bg-orange-500/10 text-orange-600 border-none px-4 py-1.5 rounded-full text-xs font-medium">Limited</Badge>
            ) : (
              <Badge className="bg-emerald-500/10 text-emerald-600 border-none px-4 py-1.5 rounded-full text-xs font-medium">Active</Badge>
            )}
          </div>
        </div>
        ...
        <div className="px-8 py-10 flex-1 space-y-10">
          ...
          <div className="flex items-center justify-between gap-6">
            <div className="flex-1">
              ...
              <div className="text-2xl font-bold text-foreground group-hover:text-amber-500 transition-colors leading-none">{route.sourceCity}</div>
              <p className="text-sm font-semibold text-amber-600 mt-2">{formatTime(departure)}</p>
            </div>
            ...
            <div className="flex flex-col items-center gap-3 flex-shrink-0">
               ...
               <span className="text-xs font-medium text-foreground bg-muted/50 px-3 py-1 rounded-full whitespace-nowrap border border-border">
                {Math.floor(route.estimatedTimeMinutes / 60)}H {route.estimatedTimeMinutes % 60}M
              </span>
            </div>

            <div className="flex-1 text-right">
              <p className="text-xs font-medium text-muted-foreground mb-2 text-right">To</p>
              <div className="text-2xl font-bold text-foreground group-hover:text-amber-500 transition-colors leading-none">{route.destinationCity}</div>
              <p className="text-sm font-semibold text-amber-600 mt-2 text-right">{formatTime(arrival)}</p>
            </div>
          </div>
          ...
          <div className="bg-muted/30 rounded-[32px] p-6 space-y-5 border border-border/50">
            <div className="flex justify-between items-end">
               <div>
                  <p className="text-xs font-medium text-muted-foreground mb-1.5">Available Seats</p>
                  <p className="text-foreground font-semibold text-base">{availableSeats} seats left</p>
               </div>
               <div className="text-right">
                  <p className="text-xs font-medium text-muted-foreground mb-1.5">Price</p>
                  <p className="text-4xl font-bold text-foreground leading-none">৳{price || bus.pricePerSeat}</p>
               </div>
            </div>
          ...
          {!isActive && (
            <div className="flex items-center gap-3 p-5 bg-destructive/5 border border-destructive/10 rounded-[24px]">
              ...
              <p className="text-sm text-destructive font-medium">Schedule Currently Unavailable</p>
            </div>
          )}
          ...
          <Link href={`/schedules/${id}`} className="block pt-2">
            <Button
              ...
              className={`w-full h-16 rounded-[24px] font-semibold text-base flex items-center justify-center gap-3 transition-all duration-500 shadow-2xl ${
                !isActive || isFull
