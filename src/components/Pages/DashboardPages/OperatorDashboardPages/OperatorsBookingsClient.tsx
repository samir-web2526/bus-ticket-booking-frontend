...
function StatusBadge({ status }: { status: BookingStatus }) {
  ...
  const Icon = cfg.icon;
  return (
    <span className={`inline-flex items-center gap-2 px-4 py-1.5 rounded-full border text-xs font-medium ${cfg.cls}`}>
      ...
      <div className="max-w-7xl mx-auto relative z-10">
        <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} className="mb-16">
          ...
          <h1 className="text-4xl lg:text-5xl font-bold text-foreground tracking-tight">
            Booking <span className="text-amber-600">Management</span>
          </h1>
          <div className="flex items-center gap-4 mt-6">
             ...
             <p className="text-muted-foreground text-sm font-medium opacity-60">
                Total bookings: <span className="text-foreground font-semibold">{bookings.length}</span>
             </p>
          </div>
        </motion.div>
            ...
            {(["ALL", "PENDING", "CONFIRMED", "EXPIRED", "CANCELLED"] as const).map((s) => {
              ...
              return (
                <button
                  ...
                  className={`px-6 py-3 rounded-2xl text-sm font-medium transition-all flex items-center gap-3 ${
                    active
                ...
                >
                  ...
                  <span className="opacity-50 font-medium text-sm">({counts[s]})</span>
                </button>
              );
            })}
          </motion.div>
        ...
        >
          <div className="grid grid-cols-[0.8fr_1.2fr_1.5fr_1fr_0.8fr_0.8fr_1fr] gap-6 px-10 py-6 border-b border-border bg-muted/10">
            {["Booking ID", "Passenger", "Route", "Bus", "Seats", "Fare", "Status"].map((h) => (
              <p key={h} className="text-muted-foreground text-sm font-medium opacity-60">{h}</p>
            ))}
          </div>
          ...
          <AnimatePresence mode="popLayout">
            {filtered.length === 0 ? (
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="py-40 text-center grayscale opacity-40">
                ...
                <p className="text-foreground font-bold text-xl tracking-tight">No bookings found</p>
                <p className="text-muted-foreground text-base font-normal mt-2">Try adjusting your search or filters</p>
              </motion.div>
            ) : (
                    ...
                    >
                      <div className="relative z-10">
                        <div className="flex items-center gap-2 mb-2">
                           ...
                           <p className="text-foreground text-sm font-medium">{booking.id.slice(0, 8)}</p>
                        </div>
                        <p className="text-muted-foreground text-xs font-medium flex items-center gap-2 opacity-50">
                      ...
                      <div className="flex items-center gap-4 min-w-0 relative z-10">
                        ...
                        <div className="min-w-0">
                          <p className="text-foreground text-base font-semibold truncate mb-1">{booking.user.name}</p>
                          <p className="text-muted-foreground text-sm font-medium truncate opacity-60">{booking.user.email}</p>
                        </div>
                      </div>
                      ...
                      <div className="flex items-center gap-4 min-w-0 relative z-10">
                        ...
                        <div className="min-w-0">
                          <p className="text-foreground text-base font-semibold truncate mb-2">
                          ...
                          <p className="text-muted-foreground text-xs font-medium opacity-50 flex items-center gap-2">
                            <Clock className="w-3 h-3" />
                            {dep.toLocaleDateString("en-BD", { month: "short", day: "numeric", hour: "2-digit", minute: "2-digit" })}
                            {isPast && <span className="text-destructive font-semibold underline underline-offset-4 ml-1">Late</span>}
                          </p>
                        </div>
                      </div>
                      ...
                      <div className="flex items-center gap-4 min-w-0 relative z-10">
                        ...
                        <div className="min-w-0">
                          <p className="text-foreground text-base font-semibold truncate mb-1 group-hover/row:text-amber-500 transition-colors">{booking.schedule.bus.name}</p>
                          <p className="text-muted-foreground text-xs font-medium opacity-50">#{booking.schedule.bus.number}</p>
                        </div>
                      </div>

                      <div className="relative z-10">
                        <p className="text-foreground text-base font-semibold mb-1">{seats}</p>
                      ...
                      <div className="flex items-center gap-3 relative z-10">
                        ...
                        <p className="text-foreground text-lg font-bold tracking-tight">৳{booking.totalFare}</p>
                      </div>
        ...
        {filtered.length > 0 && (
          <div className="mt-12 pt-10 border-t border-border/40 flex flex-col md:flex-row items-center justify-between gap-6">
             <div className="flex items-center gap-6">
                ...
                <div>
                   <p className="text-sm font-medium text-muted-foreground opacity-60 mb-1">System Status</p>
                   <p className="text-foreground font-semibold text-lg tracking-tight">Secure Connection</p>
                </div>
             </div>
             <p className="text-muted-foreground text-sm font-medium opacity-60">
               Showing <span className="text-foreground font-semibold">{filtered.length}</span> of <span className="text-foreground font-semibold">{bookings.length}</span> bookings
             </p>
          </div>
        )}
      </div>
