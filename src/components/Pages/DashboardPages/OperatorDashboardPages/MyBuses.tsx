      ...
      <div className="max-w-7xl mx-auto">
        ...
        {!loading && !error && (
          <motion.div
            ...
            className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-10"
          >
            <StatCard icon={BusFront} label="Total Buses" value={totalBuses} accent="bg-amber-400/10 text-amber-400" />
            <StatCard icon={Activity} label="Active" value={activeBuses} accent="bg-green-400/10 text-green-400" />
            <StatCard icon={AlertCircle} label="Inactive" value={inactiveBuses} accent="bg-rose-400/10 text-rose-400" />
          </motion.div>
        )}
        ...
        {!loading && !error && filtered.length === 0 && (
          <div className="flex flex-col items-center justify-center h-96 gap-4">
            <Bus className="h-16 w-16 text-slate-600" />
            <p className="text-slate-400 text-lg">
              {buses.length === 0 ? "You haven't added any buses yet." : "No buses found for this category."}
            </p>
          </div>
        )}
                ...
                >
                  ...
                  <div className="p-5">
                    <div className="flex items-start justify-between mb-4">
                      ...
                      <div>
                        <p className="text-amber-400 font-black text-xl text-right">৳{bus.pricePerSeat}</p>
                        <p className="text-slate-500 text-xs text-right">/ seat</p>
                      </div>
                    </div>
                    ...
                    <button
                      ...
                      className="w-full flex items-center justify-center gap-2 bg-amber-400/10 hover:bg-amber-400/20 border border-amber-400/20 hover:border-amber-400/40 text-amber-400 font-bold text-sm py-2.5 rounded-xl transition-all duration-200 uppercase tracking-wider mt-2"
                    >
        ...
        {!loading && !error && buses.length > 0 && (
          ...
          >
            <p className="text-slate-400 text-sm">
              Showing{" "}
              <span className="text-amber-400 font-semibold">{displayed.length}</span>
              {" "}of{" "}
            ...
            {filtered.length > 6 && (
              <Button
                onClick={() => setShowAll(!showAll)}
                variant="outline"
                className="border-white/20 text-white hover:text-amber-400 bg-white/5 hover:bg-amber-400/10 hover:border-amber-400 transition-all duration-300"
              >
                {showAll ? "Show Less" : `View All ${filtered.length} Buses`}
              </Button>
            )}
