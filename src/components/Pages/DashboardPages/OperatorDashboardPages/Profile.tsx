...
import { Mail, Phone, Shield, CheckCircle, XCircle, Building2, FileText, CreditCard, MapPin, Pencil, Loader2, Zap, Lock } from 'lucide-react';
...
const inputCls = 'w-full bg-white/5 border border-white/20 text-white rounded-xl h-11 px-3 focus:border-amber-400 focus:outline-none focus:ring-1 focus:ring-amber-400/20 transition-colors placeholder:text-slate-600';

const disabledInputCls = 'w-full bg-white/5 border border-white/20 text-slate-400 rounded-xl h-11 px-3 cursor-not-allowed opacity-60 placeholder:text-slate-600';
...
function EditOperatorModal({ user, open, onClose, onUpdated }: EditModalProps) {
  const [form, setForm] = useState({
    name: user.name || '',
    email: user.email || '',
    phone: user.phone || '',
    companyName: user.operatorProfile?.companyName || '',
    tradeLicense: user.operatorProfile?.tradeLicense || '',
    nid: user.operatorProfile?.nid || '',
    address: user.operatorProfile?.address || '',
  });
  const [loading, setLoading] = useState(false);

  // Reset form যখন modal close হয়
  useEffect(() => {
    if (open) {
      setForm({
        name: user.name || '',
  ...
  const handleChange = (field: string, value: string) => {
    // শুধু editable fields এ change হবে
    const editableFields = ['name', 'email', 'phone', 'companyName', 'address'];
  ...
  const handleSubmit = async () => {
    try {
      ...
      // শুধু editable fields send করুন
      const payload = {
        email: form.email,
        phone: form.phone,
        operatorProfile: {
      ...
      <DialogContent className="bg-[#050d1a] border border-white/10 text-white max-w-lg rounded-3xl p-0 overflow-hidden shadow-2xl">
        ...
        <DialogHeader className="px-8 pt-8 pb-5 border-b border-white/10 relative">
          <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }}>
            <p className="text-amber-400 text-xs font-semibold tracking-widest uppercase mb-1">— Edit Profile</p>
        ...
        <div className="px-8 py-6 space-y-6 relative max-h-96 overflow-y-auto">
          ...
          <div>
            <p className="text-amber-400 text-xs font-semibold tracking-widest uppercase mb-4 pb-3 border-b border-white/10">Contact Information</p>
            
            <div className="space-y-4">
              <div>
                <label className="text-sm font-bold text-amber-400 uppercase tracking-widest block mb-2 flex items-center gap-2">
                  Full Name
                </label>
                <input
                  type="text"
          ...
          <div>
            <p className="text-amber-400 text-xs font-semibold tracking-widest uppercase mb-4 pb-3 border-b border-white/10">Company Information</p>
      ...
      <div className="max-w-2xl mx-auto relative z-10">
        ...
        <div className="bg-white/[0.03] border border-white/10 rounded-2xl overflow-hidden">
          ...
          <div className="px-8 pb-8">
            ...
            <div className="mb-8">
              <p className="text-amber-400 text-xs font-semibold tracking-widest uppercase mb-4">Account Information</p>
            ...
            {user.operatorProfile && (
              <div className="mb-8">
                <p className="text-amber-400 text-xs font-semibold tracking-widest uppercase mb-4">Operator Information</p>
                <div className="space-y-3">
                  <InfoRow
                    icon={<Building2 className="w-4 h-4" />}
                    label="Company Name"
                    value={user.operatorProfile.companyName}
                  />
                  <InfoRow
                    icon={<FileText className="w-4 h-4" />}
                    label="Trade License"
                    value={user.operatorProfile.tradeLicense}
                  />
                  <InfoRow
                    icon={<CreditCard className="w-4 h-4" />}
                    label="National ID (NID)"
                    value={user.operatorProfile.nid}
                  />
                  <InfoRow
                    icon={<MapPin className="w-4 h-4" />}
                    label="Address"
                    value={user.operatorProfile.address}
                  />
                </div>
              </div>
            )}
            ...
            <div className="pt-6 border-t border-white/10 text-xs text-slate-500 flex justify-between">
              <span>
                Joined {new Date(user.createdAt).toLocaleDateString('en-BD', { year: 'numeric', month: 'long', day: 'numeric' })}
              </span>
              <span>
                Updated {new Date(user.updatedAt).toLocaleDateString('en-BD', { year: 'numeric', month: 'long', day: 'numeric' })}
              </span>
            </div>
          </div>
        </div>
