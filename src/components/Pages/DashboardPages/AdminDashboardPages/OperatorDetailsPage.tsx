  ...
  return (
    ...
    >
      <div className="px-6 py-4 border-b border-gray-100 flex items-center gap-3">
        ...
        <p className="text-gray-800 font-bold text-sm uppercase tracking-wider">{title}</p>
      </div>
  ...
  return (
    <div className="min-h-screen bg-white relative overflow-hidden p-6 lg:p-12">
      ...
      <div className="max-w-5xl mx-auto relative z-10">
        ...
        >
          <div className="h-32 bg-gradient-to-r from-amber-50 via-amber-100/60 to-transparent relative">
            ...
            <div className="-mt-12 flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4">
              <div className="flex items-end gap-5">
                {operator.profileImage ? (
                  ...
                  <div className="w-24 h-24 rounded-2xl bg-gradient-to-br from-amber-100 to-amber-200 border-4 border-white flex items-center justify-center text-amber-600 font-black text-4xl shadow-xl">
                    {operator.name?.charAt(0).toUpperCase()}
                  </div>
                )}
                <div className="pb-1">
                  <div className="flex items-center gap-3 mb-1 flex-wrap">
                    <h1 className="text-gray-900 font-black text-2xl lg:text-3xl">{operator.name}</h1>
                    <span className={`text-xs font-bold px-3 py-1 rounded-full border ${operator.status === 'ACTIVE' ? 'bg-green-50 text-green-600 border-green-200' : 'bg-red-50 text-red-500 border-red-200'}`}>
                      {operator.status}
                    </span>
                    {operator.isVerified && (
                  ...
                  {op?.companyName && (
                    <p className="text-amber-600 text-xs mt-0.5 font-medium">{op.companyName}</p>
                  )}
                </div>
              </div>
      ...
      {deleteDialog && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          >
            ...
            <h2 className="text-gray-900 font-black text-2xl mb-2">Delete Operator?</h2>
            <p className="text-gray-400 text-sm mb-8">
              Are you sure you want to delete{' '}
              <span className="text-gray-900 font-semibold">{operator.name}</span>?{' '}
